...
<!-- // Bbs bbs = new Bbs(); -->
<jsp:setProperty name="vo" property="bbsTitlen" /><!-- bbs.setBbsTitle(requrst) -->
<jsp:setProperty name="vo" property="bbsContentn" />
<%
...
<body>
	<%
		...
		if (userID == null) {
			...
			if (vo.getBbsTitlen() == null || vo.getBbsContentn() == null) {
				PrintWriter script = response.getWriter();
				script.println("<script>");
				...
				int result = dao.writeNotice(vo.getBbsTitlen(), userID, vo.getBbsContentn());
				if (result == -1) {
					PrintWriter script = response.getWriter();
					script.println("<script>");
